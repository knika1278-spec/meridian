# Meteora DLMM Screener Agent — CLAUDE.md
...
src/llm/              Provider abstraction
  ...
  claude-cli.ts       Spawns local `claude` binary
...
`manager.cron="*/10 * * * *"`, `llm.provider="claude-cli"` model `sonnet`,
...
| `LLM_PROVIDER` | no | Provider override: `claude-cli` or `mimo` |
...
| Provider | HTTP LLM | claude-cli or mimo |
